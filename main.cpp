...
static size_t buffers(4), packet_size(65507);
...
struct worker
{
  ...
  std::thread thread;
  worker(size_t myidx) : socket(service, asio::ip::udp::v4()), myremote(endpoint), read_count(0), write_count(0), thread(&worker::run, this, myidx) { }
  void doread()
  {
...
int main(int argc, char *argv[])
{
  ...
  std::cout << "Making a total of " << reads << " reads and " << writes << " writes\n";
  double rate=reads*packet_size;
